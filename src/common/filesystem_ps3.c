 ...
 *     PSL1GHT lacks of basename so as in MSVC it should be selfmade.
 *     But it had header for it there basename defined as non static.
 * 
 * -----------------------------------------------------------------------
 */
...
#if defined(_MSC_VER) || defined (__PSL1GHT__)
// looks like MSVC/the Windows CRT doesn't have basename()
...
// Posix thingy declares it as non-static
...
void FS_BuildRawPath(void) {
	...
	if (!is_portable) {
		...
		if (homedir != NULL) {
			Com_Printf("FS_AddDirToRawPath(%s) - homedir\n", homedir);
	...
	if(binarydir[0] != '\0')
	{
		Com_Printf("FS_AddDirToRawPath(%s) - binarydir\n", binarydir);
	...
	Com_Printf("FS_AddDirToRawPath(%s) - datadir\n", datadir);
